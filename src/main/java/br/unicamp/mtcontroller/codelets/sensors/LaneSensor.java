...
public class LaneSensor extends Codelet {
    ...
    public void proc() {
        ...
        if(meanVelocity != null)
            getMeanVelocityMO().updateI(Double.parseDouble(meanVelocity.toString()));


        Object maxVelocity = objectMap.containsKey(qrMaxVelocity.hashCode())? objectMap.get(qrMaxVelocity.hashCode()) : null;

        if(maxVelocity != null)
            getMaxVelocityMO().updateI(Double.parseDouble(maxVelocity.toString()));
